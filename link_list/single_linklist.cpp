...
void insertFront(struct Node **head,struct Node *ptr ,int n) 
{
	Node *newNode = new Node;
	newNode->data = n;
	newNode->next=NULL;
	 ...
	 *head = newNode;
		...
		if(ptr->next==NULL)
			ptr->next=newNode;  //插在尾巴
		else{
			newNode->next=ptr->next; //插在指定位置
			ptr->next=newNode;
		}
...
int main() 
{
	...
	addNode(head,40);
	display(head);
	cout<<"The list size is : "<< getsize(head)<<endl;

	insertFront(&head,NULL,5);
	display(head);
